defmodule Plausible.GraphQL.Resolvers.PageviewResolver do
  @moduledoc """
  ...
  def resolve_pageviews(_root, %{site_id: site_id, date_range: date_range} = args, %{site: site}) do
    with {:ok, start_date} <- parse_date(date_range.start_date),
         ...
         {data, total} <- fetch_pageviews(site, start_date, end_date, filters, args) do
      {:ok, %{
        data: data,
        pagination: build_pagination(args[:pagination], total),
        total: total
      }}
    else
  ...
  def resolve_pageviews(_root, _args, _context) do
    {:error, %{message: "Site context required"}}
  end
  ...
  defp fetch_pageviews(site, start_date, end_date, filters, args) do
    # Query existing Plausible Stats module for pageviews
    # This is a placeholder - actual implementation would use Plausible.Stats
    pagination = args[:pagination] || %{limit: 100, offset: 0}
    aggregation = args[:aggregation]

    # For now, return empty results - actual ClickHouse queries would go here
    {[], 0}
  end
