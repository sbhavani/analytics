defmodule Plausible.GraphQL.Resolvers.EventResolver do
  @moduledoc """
  ...
  def resolve_events(_root, %{site_id: site_id, date_range: date_range} = args, %{context: %{site: site}}) do
    with {:ok, start_date} <- parse_date(date_range.start_date),
         ...
         {data, total} <- fetch_events(site, start_date, end_date, filters, args) do
      {:ok, %{
        data: data,
  ...
  def resolve_events(_root, _args, _context) do
  ...
  defp fetch_events(site, start_date, end_date, filters, args) do
